...
describe('sidechat seed against the real dsh-session validator', () => {
  it('accepts a seed with a completed turn + pending user message + synthetically closed open turn', () => {
    ...
    const types = (child.snapshotEvents?.() ?? (child as any).events).map(event => event.type)
    ...
    const turnEnd = (child.snapshotEvents?.() ?? (child as any).events).findLast(event => event.type === 'turn/end')
  ...
  it('accepts a dangling-tool-call fallback seed (cut before the open turn)', () => {
    ...
    expect((child.snapshotEvents?.() ?? (child as any).events).map(event => event.type).at(-1)).toBe('session/end-seed')
  })
