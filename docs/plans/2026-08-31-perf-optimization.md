...
**目标宿主**：DSH v0.1.2-alpha.2（`/Users/menghuan/Code/deepseek-harness` @ `dsh-v0.1.2-alpha.2`，只读参考）
**测量环境**：macOS arm64 本机，真实 `dsh web`（npx 钉 `@deepseek-ai/dsh@0.1.2-alpha.2`）+ scratch profile 挂载 tarball，Playwright Chromium 无头，每指标 3 次取中位数（`tests/e2e/perf.e2e.ts` 输出 `PERF_JSON`）。
