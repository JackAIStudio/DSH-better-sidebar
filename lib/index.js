...
async function threadLogEvents(ctx, childId) {
	...
	if (agent !== void 0) return readSessionEvents(agent.session);
	const persistence = ctx.get("sessionPersistence");
...
function buildSidechatApi(ctx) {
	return {
		"sidechat.start": async (payload) => {
			...
			const { agentPreset, setup } = await composeChildSetup(ctx, resolvePresetId(parentSession.header, readSessionEvents(parentSession)));
		...
		"sidechat.prompt": async (payload) => {
			...
			if (boundaryDelivered(readSessionEvents(agent.session))) admitFollowup(agent, textPrompt(text));
			else {
